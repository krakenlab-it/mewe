...
export function DashboardDaughterPage({ dupla, onBack, onLogout, onStartTest, onViewReport, onGoCrisis, onGoPolicy }) {
  const h = dupla.hija;
  const answered = Object.keys(h.respuestas || {}).length;
  const totalDaughter = PREGUNTAS.hija.length;
  return (
    <Shell>
      <TopBar title={`Hola, ${h.nombre || ""} 🌱`} onBack={onBack} onLogout={onLogout} />
