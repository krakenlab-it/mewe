...
interface PhotoUploaderWithCloudProps {
  userId: string;
  onUploadComplete?: (uploadedFiles: UploadedFile[]) => void;
  buttonText?: string;
  buttonClassName?: string;
  maxFiles?: number;
  activityId?: string; // Add activityId prop
}
...
export function PhotoUploaderWithCloud({
  userId,
  onUploadComplete,
  ...
  const handlePhotoSelect = async (files: File[]) => {
      ...
      try {
        ...
        const metadataResponse = await fetch(`/api/users/${userId}/files`, {
          ...
          body: JSON.stringify({
            ...
            activityId: activityId, // Include activityId if provided
          }),
        });
    ...
    if (uploadedFiles.length > 0) {
      toast({
        ...
        description: `${uploadedFiles.length} foto(s) subida(s) a la nube.`,
      });
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">
              Subiendo fotos {isUploading && <Loader2 className="inline w-4 h-4 ml-2 animate-spin" />}
            </CardTitle>
          </CardHeader>
