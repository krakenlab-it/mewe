-- Make code-based pair access resilient across anonymous auth sessions.
-- The pair code is the shared secret for returning to a participant role.
...
begin
  ...
  -- A valid pair code can be used from a fresh anonymous session. Move the
  -- role binding to the current auth user instead of failing on the unique
  -- participant_id constraint.
